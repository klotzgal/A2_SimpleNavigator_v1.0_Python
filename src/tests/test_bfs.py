...
def test_valid(non_weighted_graph: Graph) -> None:
    alg = GraphAlgorithms()
    way = alg.breadth_first_search(non_weighted_graph, 0)
    assert way == [0, 1, 2, 3, 5, 7, 4, 6]


def test_invalid(non_weighted_graph: Graph) -> None:
    alg = GraphAlgorithms()
    with pytest.raises(IndexError):
        alg.breadth_first_search(non_weighted_graph, 10)


def test_non_full_0(non_full_graph: Graph) -> None:
    alg = GraphAlgorithms()
    way = alg.breadth_first_search(non_full_graph, 0)
    assert way == [0, 1, 2, 3, 5, 4, 6]


def test_non_full_7(non_full_graph: Graph) -> None:
    alg = GraphAlgorithms()
    way = alg.breadth_first_search(non_full_graph, 7)
    assert way == [7]


def test_loop_0(loop_graph: Graph) -> None:
    alg = GraphAlgorithms()
    way = alg.breadth_first_search(loop_graph, 0)
    assert way == [0, 1, 2, 3, 5, 7, 4, 6]


def test_loop_3(loop_graph: Graph) -> None:
    alg = GraphAlgorithms()
